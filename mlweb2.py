# -*- coding: utf-8 -*-
"""
Created on Tue Jul 19 10:42:11 2022
...
Pregnency_model=pickle.load(open('Pregnency_model.sav','rb'))
...
#Pregnency risk prediction page
if(selected=='Pregnency Risk Prediction'):

    #page title
    st.title('Pregnency Risk Prediction using machine learnig')
    ...
    with col3:
        DiastolicBP = st.text_input('DiastolicBPvalue')
        
        
    with col1:
       BS = st.text_input('Bs value')
    ...
    if st.button('pregnency Test Results'):
        Pregnency_prediction= Pregnency_model.predict([[Age,SystolicBP,DiastolicBP, BS,BodyTemp ,HeartRate]])
        if( Pregnency_prediction[0]==0):
            Pregnency_diagnosis=' low risk'
        else:
            Pregnency_diagnosis='high risk'
            
    st.success(Pregnency_diagnosis)  
